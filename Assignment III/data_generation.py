...
plug_id = get_all_plug()
for _ in range(1000):
    cursor.execute('INSERT INTO car_charging_log (creating_time, price,'
                   ' car_license_plate, charging_station_id, plug_id) '
                   'VALUES (%s, %s, %s, %s, %s)',
                   [random_date('1/1/2018 1:00 AM', '3/1/2018 1:00 AM', random.random()), random.randint(5, 15),
...
"""CAR PARTS ORDER"""
cursor = connection.cursor()
car_parts_id = get_all_car_parts()
workshop_id = get_all_workshops()
car_provider_id = get_all_providers()
for _ in range(10000):
    cursor.execute(
        'INSERT INTO car_parts_order (description, status, creating_time, amount, '
        'car_part_id, workshop_id, car_parts_provider_id) VALUES (%s, %s, %s, %s, %s, %s, %s)',
        ['No description', 0, random_date('1/1/2018 1:00 AM', '3/1/2018 1:00 AM', random.random()), random.randint(5, 50),
         random.choice(car_parts_id), random.choice(workshop_id), random.choice(car_provider_id)])
cursor.close()
connection.commit()
...
car_part_id = get_all_car_parts()
workshop_id = get_all_workshops()
manager_username = get_all_manager_username()
car_license_plate = get_all_cars()
for _ in range(30):
    cursor.execute(
        'INSERT INTO car_repairing_log (date, manager_username, workshop_id, car_license_plate, broken_car_part_id) '
        'VALUES (%s, %s, %s, %s, %s)',
        [random_date('1/1/2018 1:00 AM', '3/1/2018 1:00 AM', random.random()), random.choice(manager_username),
         random.choice(workshop_id), random.choice(car_license_plate),
         random.choice(car_part_id)])
cursor.close()
connection.commit()
